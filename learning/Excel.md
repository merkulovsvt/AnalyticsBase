...
* [Курс](https://practicum.yandex.ru/profile/excel-for-work/) на Яндекс Практикуме.
...
* `=COUNTIFS(диапазон критерия 1; значение критерия 1; ...)` — количество по критериям.
    * Значение критерия может быть и условием вида `"<5"`, `"<>0"`, `"<="&D8` ...
...
* `=MINIFS(диапазон вычисления минимума; диапазон_критерия1; критерий1; ...)` — минимум по критериям.
    * Значение критерия может быть и условием вида `"<5"`, `"<>0"`, `"<="&D8` ...
...
* `=AND(условие1; условие2; ...)` — логические **И**.


* `=OR(условие1; условие2; ...)`— логические **ИЛИ**.


* `=IF(условие для проверки; результат, если истина; результат, если ложь)` — условие **ЕСЛИ**.


* `=VLOOKUP(искомое значение; диапазон поиска; номер столбца в диапазоне с возвращаемым значением; {0|1})` — ВПР.
    * **0** четвертым параметром — точное совпадение (используется чаще всего).
    * **1** четвертым параметром — приблизительное совпадение (поиск ближайшего к искомому значению).


* `=IFERROR(формула, которая может давать ошибку; ответ, чтобы показывать его при ошибке)` — оборачивает формулу,
  которая выдаёт ошибку, и задаёт ей новый ответ.


* `=IMPORTRANGE("URL на файл"; "диапазон ячеек в файле")` — импортирует диапазон одной таблицы в другую.
    * Оба параметра обязательно должны быть в кавычках.


* `=QUERY(диапазон данных; "SQL-запрос")` — формирование **SQL-запроса** внутри **Excel**.
    * Внутри **SQL-запроса** названия столбцов обязательно с заглавных букв.


* `=SPARKLINE(диапазон данных)` — создать линию тренда на основе данных из диапазона.
...
* <u>Выделить диапазон данных</u> → **Данные** → **Разделить текст на столбцы**.
...
* <u>Выделить диапазон данных</u> → `CTRL + C` → `ПКМ` → **Специальная вставка** → **Вставить только значения**.
...
* Почти то же самое, что и `LEFT JOIN` в **SQL**.


* `=VLOOKUP(искомое значение; диапазон поиска; номер столбца в диапазоне с возвращаемым значением; {0|1})`.
    * **0** четвертым параметром — точное совпадение (используется чаще всего).
    * **1** четвертым параметром — приблизительное совпадение (поиск ближайшего к искомому значению).


* Всегда ищет значение в первом столбце указанного диапазона.

* Когда `VLOOKUP` находит первое совпадение с запросом в диапазоне поиска, то оно прекращает поиск.

* Выводит `#N/A` (можно заменить, используя `IFERROR`), если искомое значение не найдено.
...
### Импорт части данных (SQL-запросы)
...
* Если таблица большая, то при её листании становится непонятно, к чему относятся числа.


* <u>Выделить диапазон данных</u> → **Вид** → **Закрепить**

#### Чередование цветов

* Водить глазами по длинным строкам с множеством цифр тяжело: строчки путаются, а нужная ячейка теряется.


* <u>Выделить диапазон данных</u> → **Формат** → **Чередование цветов**

#### Спарклайны

* Для данных за определённый период можно построить линию тренда — визуальное отображение динамики их
  изменений.
...
* Визуальные акценты (цвет, шрифт, жирность, ...) повысят информативность данных.


* <u>Выделить диапазон данных</u> → **Формат** → **Условное форматирование**
    * Есть два формата условного форматирования — **Один цвет** и **Градиент**.

### Графики и диаграммы

* Выбор диаграммы лучше начинать с вопроса — **Какое отношение между величинами?**
    * Читать на [Практикуме](https://practicum.yandex.ru/profile/excel-for-work/).

#### График

* Главное предназначение — показать, как с течением времени (или ростом другого параметра) изменяются данные.

#### Столбчатые диаграммы

* Главное предназначение — сравнить дискретные (прерывистые) величины.

#### Круговые диаграммы

* Главное предназначение — показать соотношение частей одного целого, взятого за 100%.